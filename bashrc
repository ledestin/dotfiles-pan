...
PATH=/usr/local/bin:/usr/bin:/bin:/usr/local/sbin:/usr/sbin:/sbin:/usr/bin/X11:/usr/X11R6/bin:/usr/games:/snap/bin:~/.bin
...
#  }
  ...
  alias gc='git commit'
  alias gd='git diff'
  alias gdw='git diff --word-diff --word-diff-regex=.'
  alias gdc='gd --cached'
  alias gdcw='gd --cached --word-diff --word-diff-regex=.'
  alias gs='git show'
  alias gst='git st'
  alias grs='git reset'
...
[ -f $HOME/.bash_credentials ] && . $HOME/.bash_credentials
